...
public class addDataScenario {
    ...
    @And("^the location \"(.*?)\" exists in the airport table if the location_type \"(.*?)\" is \"APT\"$")
    public void locationExistsLocationTypeIsAPT(String location, String location_type) {
        if (location_type.equals("APT")) {
            Assert.assertTrue(airportAccessor.dataExists(location));
        }
    }
    ...
    public void flightEntryAdded(int flight_id, int id) {
        ArrayList<Integer> expected_ids = new ArrayList<>(Arrays.asList(1, 1));
    ...
    @And("^the airline code \"(.*?)\" exists in the airline table$")
    public void airlineExists(String airline) {
        Assert.assertTrue(airlineAccessor.dataExists(airline));
    }

    @And("^the source airport code \"(.*?)\" exists in the airport table$")
    public void sourceAirportExists(String source_airport) {
        Assert.assertTrue(airportAccessor.dataExists(source_airport));
    }

    @And("^the destination airport code \"(.*?)\" exists in the airport table$")
    public void destinationAirportExists(String dest_airport) {
        Assert.assertTrue(airportAccessor.dataExists(dest_airport));
    }
